...
public class Student extends Person
{
	...
	public Student(String firstName, String lastName, String midInitial, 
		       String IDNum, String address, String homePhone, char gender,
		       String major, char program, boolean hasGraduated)
	{
	...
	{
		return IDNum+" "+name.firstName+" "+name.lastName+" Major: "+major;
	}
...

	{
		...
		Student s = (Student) q.execute(ID);
	...
	public static Student find_(String ID, PersistenceManager pm)
	...
	public static Collection<Student> sameLastName1_(Query q)
...

	{
		...
		q = pm.newQuery(
                    "select from Student " + 
		    "where this.name.lastName == s1.name.lastName &&" +
		          "this != s1 " +
	...
	public static Collection<Student> sameLastName2_(Query q)
	...
	public Collection<CourseSection> getCourseSections_(Query q)
...

	{
		...
		q = pm.newQuery(
		    ...
		    "where this.take.contains(t) && t.student == s " +
		    "variables Take t " +
		    "parameters Student s " +
		    "order by this.year ascending, this.semester ascending, this.registrationCode ascending");
	...
	public static void main(String argv[])
	{
		...
		Student s = Student.find_(argv[0], pm);
		if ( s == null )
		{
		...
		Query q = pm.newQuery();
		Utility.printCollection(s.getCourseSections_(q));
		q.closeAll();
		System.out.println();

		q = pm.newQuery();
		Utility.printCollection(Student.sameLastName1_(q));
		q.closeAll();
		System.out.println();

		q = pm.newQuery();
		//System.out.println(Student.sameLastName2_(q).size());
